...
export default function AddEntryPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (locationId) {
                router.push(`/dashboard/trips-overview/${tripId}/location/${locationId}`);
            } else {
                router.push(`/dashboard/trips-overview/${tripId}`);
            }
        } catch (err: unknown) {
            type ErrLike = { response?: { data?: { message?: string } }; message?: string };
            const anyErr = err as ErrLike;
            const msg = anyErr?.response?.data?.message || anyErr?.message || 'Failed to create entry';
                ...
                <CardContent className="space-y-4">
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                            ...
                            <Textarea id="content" value={description} onChange={(e) => setDescription(e.target.value)} />
                        </div>
                        ...
                        <div>
                            ...
                            <div className="mt-2 flex items-center gap-3">
                                ...
                                <span className="text-sm text-muted-foreground">You can upload multiple images (max handled by server)</span>
                            </div>
                            ...
                            {previews.length > 0 && (
                                <>
                                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-3">
                                        {previews.map((p, idx) => (
                                            <div key={p.url} className="relative rounded overflow-hidden border cursor-pointer" onClick={() => openLightbox(idx)}>
                                                <Image src={p.url} alt={`preview-${idx}`} width={400} height={200} unoptimized className="object-cover w-full h-32" />
                                                <button type="button" onClick={(ev) => { ev.stopPropagation(); removePreview(idx); }} className="absolute top-1 right-1 bg-white/80 rounded p-1">
